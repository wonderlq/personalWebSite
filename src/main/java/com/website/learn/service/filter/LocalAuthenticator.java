...
public class LocalAuthenticator implements Authenticator {
    ...
    private UserDetail getUserInfoByCookie(String cookie) {
        ...
        //这个cookie里加密存储了id和name,格式“id-time”
        String[] s = AESUtil.decrypt(cookie,ValueConstant.COOKIE_PASSWORD).split("-");
        UserBasePo userBasePo = userService.findProfileById(Long.valueOf(s[0]));
        return UserBuilder.userInfoPo2UserDetail(userBasePo);
    }
